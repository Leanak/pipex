...
/*   Updated: 2025/08/09 18:26:04 by lenakach         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	first_child(t_pipex *pipex, char **av, char **envp)
{
	...
	if (!pipex->cmd_args || !pipex->cmd_args[0])
		error_exit1(pipex, "Child 1 : Split cmd args failed");
	pipex->cmd = get_cmd(pipex->cmd_args[0], envp);
	if (!pipex->cmd)
		error_exit127(pipex, "child 1: Not finding command with access");
...
void	second_child(t_pipex *pipex, char **av, char **envp)
{
	...
	if (!pipex->cmd_args)
		error_exit1(pipex, "child 2 :Split cmd args failed");
	pipex->cmd = get_cmd(pipex->cmd_args[0], envp);
	if (!pipex->cmd)
		error_exit127(pipex, "child 2 : Not finding command with access");
	execve(pipex->cmd, pipex->cmd_args, envp);
	perror("Execve not working");
	free_all(pipex);
	exit(1);
